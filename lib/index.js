...
const endpointSchema = Schema.object({
  ...
  /** Optional recharge/top-up page URL opened when the balance is clicked. */
  rechargeUrl: Schema.string(),
})
...
export const Config = Schema.object({
  ...
  endpoints: Schema.array(endpointSchema).default([
    ...
    },
    {
      ...
      balancePath: 'data.available_balance',
      rechargeUrl: 'https://platform.moonshot.ai/',
    },
    {
      id: 'minimax',
      name: 'MiniMax',
      ...
      rechargeUrl: 'https://platform.minimaxi.com/',
    },
    {
      id: 'stepfun',
      name: 'StepFun',
      ...
      rechargeUrl: 'https://platform.stepfun.com/',
    },
    {
      id: 'zhipu',
      name: 'Zhipu/GLM',
      ...
      rechargeUrl: 'https://open.bigmodel.cn/',
    },
  ]),
})
...
async function fetchBalance(provider) {
  const { id, name, url, balancePath, currencyPath, auth, key, rechargeUrl } = provider
  try {
    const res = await fetch(url, {
      headers: { Authorization: `${auth} ${key}` },
      signal: AbortSignal.timeout(10000),
    })
    if (!res.ok) return { id, name, error: `http-${res.status}`, ...rechargeUrl !== undefined ? { rechargeUrl } : {} }
    const json = await res.json()
    const balance = getPath(json, balancePath)
    if (balance === undefined) return { id, name, error: 'missing-path', ...rechargeUrl !== undefined ? { rechargeUrl } : {} }
    return {
      id,
      name,
    ...
    return { id, name, error: String(e?.message ?? e), ...rechargeUrl !== undefined ? { rechargeUrl } : {} }
  }
}
...
/**
 * Recharge/console URLs for providers with no public balance API, keyed by
 * their credential ref. Clicking the "no-api" label opens these.
 */
const NO_API_RECHARGE = {
  DASHSCOPE_API_KEY: 'https://bailian.console.aliyun.com/',
  OPENAI_API_KEY: 'https://platform.openai.com/settings/organization/billing',
    ...
    try {
      ...
      for (const provider of ctx.llm.listProviders()) {
        ...
        providers.push({
          ...
          name: provider.name,
          error: 'no-api',
          ...NO_API_RECHARGE[apiKeyEnv] !== undefined ? { rechargeUrl: NO_API_RECHARGE[apiKeyEnv] } : {},
        })
      }
