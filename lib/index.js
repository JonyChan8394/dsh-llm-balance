 ...
 *
 * Providers are config-driven so any LLM API with a balance endpoint can be
 * added; presets ship for DeepSeek, OpenRouter, SiliconFlow, Moonshot/Kimi,
 * MiniMax, StepFun and Zhipu/GLM. A provider whose key is missing reports
 * `no-key`; one whose endpoint rejects the key reports the HTTP status.
 *
 * Note: Qwen/DashScope (aliyun) exposes no public balance API — the balance
 * is only visible in the console, so no preset exists for it.
 */
...
async function fetchBalance(ctx, provider) {
  const { id, name, apiKeyRef, url, balancePath, currencyPath, auth } = provider
  const key = await resolveKey(ctx, apiKeyRef)
  if (!key) return { id, name, error: 'no-key' }
  try {
  ...
  const handler = async (req, res) => {
    try {
      ...
      for (const provider of config.providers) {
        providers.push(await fetchBalance(ctx, provider))
      }
