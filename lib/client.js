...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function BalanceDock({ t }) {
			...
			// Clicking a provider opens its recharge/top-up page in a new tab.
			const openRecharge = (url) => () => {
			...
			return react.createElement(
				...
				shown.map((p) =>
					react.createElement(
						...
						{
							key: p.id,
							title: p.rechargeUrl || p.name,
							onClick: openRecharge(p.rechargeUrl),
							style: {
								display: 'inline-flex', alignItems: 'center', gap: '4px',
								cursor: p.rechargeUrl ? 'pointer' : 'default',
							},
						},
